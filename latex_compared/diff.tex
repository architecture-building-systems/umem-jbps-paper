...
%DIF DEL UMEM_CoSim_original.tex    Mon Dec 19 00:47:32 2016
%DIF ADD UMEM_CoSim_corrected.tex   Wed Apr 26 06:25:43 2017
...
\begin{abstract}
    ...
    to the both the CitySim simulation \DIFdelbegin \DIFdel{and EnergyPlus engines}\DIFdelend \DIFaddbegin \DIFadd{engine}\DIFaddend . The results
    ...
    heating consumption \DIFaddbegin \DIFadd{as compared to solo simulations of each of the engines}\DIFaddend . Challenges encountered with the urban scale
...
2.5 billion people are expected to join cities throughout the world \DIFaddbegin \DIFadd{by the year 2030
}\DIFaddend \citep{UnitedNations:2014vn}. Expansions of entire districts and even cities
...
results in certain deficiencies concerning modeling various \DIFdelbegin \DIFdel{phenomenon}\DIFdelend \DIFaddbegin \DIFadd{phenomena}\DIFaddend .
...
Building Energy Simulation (BES) model. \DIFaddbegin \DIFadd{The scales and models contained in the 
...
\begin{enumerate}
  \item \DIFdelbegin \DIFdel{Automation }\DIFdelend \DIFaddbegin \DIFadd{Automate }\DIFaddend of the process of simultaneous meta-data extraction from a
  ...
  \item Implement a simplified calibration procedure \DIFdelbegin \DIFdel{based on }\DIFdelend \DIFaddbegin \DIFadd{for the first scenario by 
  }\DIFaddend reconciling the co-simulation output results for a target building using measured energy
...
\citep{Martin:2015fj}. \DIFaddbegin \DIFadd{While not directly coupled, }\DIFaddend EnergyPlus and the TEP Urban Canopy Model program were \DIFdelbegin \DIFdel{coupled }\DIFdelend \DIFaddbegin \DIFadd{connected
through a modified weather file }\DIFaddend to quantify the influences of urban localized weather effects
...
of the building information model from the earliest design possible \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend can be
...
geography and climate conditions. These variables \DIFaddbegin \DIFadd{are outlined in Table \ref{FMUimports}. 
They }\DIFaddend include various outdoor air
conditions such as dry bulb and wet bulb temperatures, \DIFdelbegin \DIFdel{dewpoint}\DIFdelend \DIFaddbegin \DIFadd{dew-point}\DIFaddend , humidity, wind
...
urban environments \DIFdelbegin \DIFdel{, }\DIFdelend that are often neglected in building-scale simulation
...
targeted building. \DIFaddbegin \DIFadd{Table \ref{FMUexports} outlines this list of exchanged variables.
}\DIFaddend These variables are transferred due to the ability of
...
readily available simulation results. An automated work flow \DIFdelbegin \DIFdel{to tie }\DIFdelend \DIFaddbegin \DIFadd{ties }\DIFaddend the
...
The focus of this case study is to \DIFdelbegin \DIFdel{compare }\DIFdelend \DIFaddbegin \DIFadd{quantify }\DIFaddend the impact of the information
...
this study was to compare the \DIFaddbegin \DIFadd{solo and co-simulation }\DIFaddend output results of \DIFdelbegin \DIFdel{both CitySimand EnergyPlus}\DIFdelend \DIFaddbegin \DIFadd{CitySim}\DIFaddend .
...
%DIF <  An existing EnergyPlus model is used for the HPZ building and simplified single-zone EnergyPlus models are extracted from the CitySim geometry for the other three targeted buildings.

\begin{figure}[H]
\centering
\includegraphics[scale=0.2]{figures/HPI_Campus.png}
\caption{The HPI building \DIFdelbeginFL \DIFdelFL{modeled }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EnergyPlus model extracted from the CitySim model. The HPI building is seen
as the light and dark brown building in the center surrounded by gray masses that represent other buildings on 
campus. The graphic is illustrated }\DIFaddendFL with OpenStudio for \DIFdelbeginFL \DIFdelFL{Sketchup}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Sketch-up.}\DIFaddendFL }
\label{fig:HPIcampus}
\end{figure}
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Measured data collection and calibration}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Energy data from the HPI building was collected and analyzed in the context of
the simplified calibration procedures. The measured datasets \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend screened and
...
clustering process known as \emph{DayFilter} \citep{Miller:2015kr}. \DIFdelbegin \DIFdel{This
profile is }\DIFdelend \DIFaddbegin \DIFadd{Cluster 0 represents
...
non-summer months. These profiles are }\DIFaddend used to set the availability schedules as inputs into the
...
\caption{\DIFdelbeginFL \DIFdelFL{HPI Electrical Energy }\DIFdelendFL Typical \DIFdelbeginFL \DIFdelFL{Profiles}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{measured data electricity profiles of (left) average daily electrical consumption for each cluster and (right)
...
a day of the week (starting with Monday) and each column corresponding to a week of the month.}\DIFaddendFL }
\label{fig:hpi_elec_profiles}
\end{figure}

The measured and simulated data from both the co-simulation and EnergyPlus
solo simulation \DIFdelbegin \DIFdel{was compared }\DIFdelend \DIFaddbegin \DIFadd{were analyzed }\DIFaddend in order to \DIFdelbegin \DIFdel{create a realistic scenario for
comparison}\DIFdelend \DIFaddbegin \DIFadd{understand how both the solo and co-simulations 
compare to real, measured data}\DIFaddend . The calibration process was undertaken through a series of steps
...
heating. \DIFdelbegin \DIFdel{This approach was taken as the data quality for the measurements were
deemed most accurate during the appropriate seasons. }\DIFdelend Figure \ref{fig:hpi_measvssim} illustrates a comparison of the measured data with the
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Campus case study 2: EPFL Quartier Nord} 
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Model development}
...
%DIFDELCMD < %%%
%DIF <  \section{Results}
\DIFdelend \subsection{Comparison of EnergyPlus and CitySim}
...
\DIFadd{Two campus case studies were presented in this paper to showcase a novel process of extracting 
...
\DIFadd{Advantages of a co-simulation process between EnergyPlus and CitySim were uncovered through
...
\DIFadd{Disadvantages of the co-simulation process are mainly centered upon the increase
...
\subsection{\DIFdelbegin \DIFdel{Acknowledgements}\DIFdelend \DIFaddbegin \DIFadd{Acknowledgments}\DIFaddend }
